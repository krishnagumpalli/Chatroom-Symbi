...
public class Server {
    ...
    public void run() throws IOException {
        ...
        while (true) {
            ...
            // adds newUser's message to list
            this.clients.add(newUser);

            // Welcome message
            newUser.getOutStream().println(
                    "<img src='https://1.bp.blogspot.com/-P9k8T28Hpbc/XcA2AEC9azI/AAAAAAAACVM/Dcw7w5kaLjQasUMkz21-LCpgVi_ZOcu4ACLcBGAsYHQ/s1600/1kitty.gif' height='42' width='42'>"
                            + "<b>Welcome</b> " + newUser.toString() +
                            "<img src='https://1.bp.blogspot.com/-P9k8T28Hpbc/XcA2AEC9azI/AAAAAAAACVM/Dcw7w5kaLjQasUMkz21-LCpgVi_ZOcu4ACLcBGAsYHQ/s1600/1kitty.gif' height='42' width='42'>"
            );

            // creates a new thread for handling newUser's incoming messages
    ...
    // sends incoming message to all the users
    ...
    // sends list of clients to all users
...
class UserHandler implements Runnable {
    ...
    public void run() {
        ...
        while (sc.hasNextLine()) {
            ...
            // Managing the sending of private messages
            if (message.charAt(0) == '@'){
                if(message.contains(" ")){
                ...
                //To change colour in which PRN appears
            }else if (message.charAt(0) == '#'){
                user.changeColor(message);
                // updates the colour for all other users
                this.server.broadcastAllUsers();
            }else{
                // updates user list
...
class User {
    ...
    //to change the colour of user's PRN
    public void changeColor(String hexColor){
        ...
        if (m.matches()){
            ...
            // if the Colour is too Bright it doesn't change
            double luma = 0.2126 * c.getRed() + 0.7152 * c.getGreen() + 0.0722 * c.getBlue(); // per ITU-R BT.709
            if (luma > 160) {
                this.getOutStream().println("<b>Colour is Too Bright</b>");
                return;
            }
            this.color = hexColor;
            this.getOutStream().println("<b>Colour changed successfully</b> " + this.toString());
            return;
        }
        this.getOutStream().println("<b>Failed to change colour</b>");
    }
